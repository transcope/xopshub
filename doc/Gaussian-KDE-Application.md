...
* 带宽会影响模型输出（曲线）的平滑程度。具体地，
  * 当带宽越大，容易造成欠平滑（undersmoothing），说明观察到的数据点在最终形成的曲线形状中所占比重越小；
  * 当带宽越小，容易造成过平滑（oversmoothing），说明观察到的数据点在最终形成的曲线形状中所占比重越大；


<p align="center">
  <img src="../image/kde-application/kde_smoothing.png" width="600"/>
  </br>图1-3：(a) 欠平滑，曲线的波峰过多；(b) 过平滑，曲线只有一个波峰；（c）这是合理的参数结果。
</p>
